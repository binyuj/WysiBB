					...
					modal: {
						...
						tabs: [
							...
							},
							{
								...
								html: '<iframe src="http://www.qiannao.com/upload/dzupload.html?userId=yubin1991&amp;return_url=https://binyuj.me/content/plugins/diyface/upload_return_url.php" scrolling="no" frameborder="0" align="center" width="389" height="170"></iframe></div>'
							}
						],
		...
		showModal: function(cmd,opt,queryState) {
			...
			if (opt.tabs && opt.tabs.length>1) {
				...
				$.each(opt.tabs,$.proxy(function(i,row) {
					if (i==0) {row['on']="on"}
					$ul.append(this.strf('<li class="{on}" onClick="$(this).parent().find(\'.on\').removeClass(\'on\');$(this).addClass(\'on\');$(this).parents(\'.wbbm-content\').find(\'.tab'+i+'\').show()">{title}</li>',row));
